...
def progressive_val_score(
    ...
    **print_kwargs,
    ...
    #adaption: code not needed is commented out, since no msg on the consol wanted
    #active_learning = utils.inspect.isactivelearner(model)
    
    #adaption: variables necessary to also return the time and memory consumption of the computation
    time = None
    memory = None
    ...
    for checkpoint in checkpoints:
        ...
        if show_time:
            ...
            #adaption: give the time of computation to the time variable
            time = f"{H:02d}:{M:02d}:{S:02d}"
        if show_memory:
            #msg += f" – {utils.pretty.humanize_bytes(checkpoint['Memory'])}"
            #adaption: give the memory consumption to the memory variable
            memory = f"{utils.pretty.humanize_bytes(checkpoint['Memory'])}"
        #print(msg, **print_kwargs)
    
    #adaption: additional return parameter time and memory added
    return metric, time, memory
