...
class DriftRetrainingClassifier(base.Wrapper, base.Classifier):
    ...
    def __init__(
        ...
        #adaption: attributes added 
        self.position = 0 #attribute to track the current position in the evaluated dataset
        self.drifts = [] #list that tracks the position of the detected concept drifts
    ...
    def _update_detector(self, x, y):
        ...
        if self.train_in_background:
            if self.drift_detector.warning_detected:
                ...
                #adaption: update of attribute added
                self.drifts.append(self.position)
    ...
    #adaption: method added, returns the number of detected drifts
    def get_n_drifts(self):
        return len(self.drifts)

    #adaption: method added, returns a list that contains the positions of all detected drifts
    def get_drifts(self):
        return self.drifts
    
    #adaption: method added, reset the drift detector
