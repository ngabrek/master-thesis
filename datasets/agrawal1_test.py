...
def get_abrupt_agrawal1_datasets(size:int, seed:int=23, n:int=30) -> dict:
    datasets = {}
    name = str(size/1000)
    
    for i in range(0,n):
        datasets['agrawal1_abrupt_' + name + 'K_'+str(i)] = (create_abrupt_agrawal1_dataset(size, seed+i), size)
        
    nominal_features = ['elevel','car','zipcode']
    
    return datasets, nominal_features
        
def get_gradual_agrawal1_datasets(size:int, seed:int=23, n:int=30) -> dict:
    datasets = {}
    name = str(size/1000)
    
    for i in range(0,n):
        datasets['agrawal1_gradual_' + name + 'K_'+str(i)] = (create_gradual_agrawal1_dataset(size, seed+i), size)
