...
gradual, nominal_features = get_agrawal_datasets(2,size, gradual=True)

#evaluate the gradual datasets on KSWIN with the BOLE classifier and then write the results in a csv-file  
i = 151
gradual_bole = evaluation(gradual,nominal_attributes=nominal_features,nb=False,majclass=False,nochange=False,ht=False,basic=False,adwin=False,cusum=False,ddm=False,
                           ecdd=False,eddm=False,gma=False,hddma=False,hddmw=False,bocd=False,ph=False,rddm=False,stepd=False,seed=i)
write_results_to_csv(f'results/{generator}/{size_name}/{generator}_{size_name}_gradual_bole_kswin_{i}.csv', gradual_bole)
