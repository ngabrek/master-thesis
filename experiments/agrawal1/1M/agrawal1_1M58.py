...
#evaluate the gradual datasets on CUSUM with the BOLE classifier but BOCD and KSWIN and then write the results in a csv-file  
i = 151
gradual_bole = evaluation(gradual,nominal_attributes=nominal_features,
                          nb=False,majclass=False,nochange=False,ht=False,bocd=False,basic=False,adwin=False,ddm=False,
                          ecdd=False,eddm=False,gma=False,hddma=False,hddmw=False,kswin=False,ph=False,rddm=False,stepd=False,seed=i)
write_results_to_csv(f'results/{generator}/{size_name}/{generator}_{size_name}_gradual_bole_cusum{i}.csv', gradual_bole)  
